...
<script>
	//if new account pages
			...
			//signIn upodater JS below
                //
                <% if (session.getAttribute("pageName") != "SignIn.jsp"){ %>
			var signInReturn =document.getElementsByName('lateLogin.xURL');
			$(signInReturn).val('/jsp/<%= session.getAttribute( "returnPage" ) %>');
			//
            ...
            <%}%>
                //
			//Alerts override
             var errorMSGtxt = "<webbase:errorData dataType='ErrorMsg'/>"
				if (errorMSGtxt!=""){
					$("#EpageError").show().text(errorMSGtxt);
				};
				//
