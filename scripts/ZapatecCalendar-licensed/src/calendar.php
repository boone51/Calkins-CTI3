<?php
/* $Id: calendar.php 4322 2006-09-04 08:49:33Z shacka $ */
...
class DHTML_Calendar {
    ...
    function get_load_files_code() {
        $code  = ( '<link rel="stylesheet" type="text/css" media="all" href="' .
                   $this->calendar_lib_path . 'themes/' . $this->calendar_theme_file .
                   '" />' . NEWLINE );
        $code .= ( '<script type="text/javascript" src="' .
                   $this->calendar_lib_path . 'src/' . $this->calendar_file .
                   '"></script>' . NEWLINE );
        $code .= ( '<script type="text/javascript" src="' .
                   $this->calendar_lib_path . $this->calendar_lang_file .
                   '"></script>' . NEWLINE );
        $code .= ( '<script type="text/javascript" src="' .
                   $this->calendar_lib_path . 'src/' . $this->calendar_setup_file .
                   '"></script>' );
        return $code;
    }
    ...
    function _make_calendar($other_options = array()) {
        ...
        $code  = ( '<script type="text/javascript">Zapatec.Calendar.setup({' .
                   $js_options .
                   '});</script>' );
        return $code;
    }
    ...
    function make_input_field($cal_options = array(), $field_attributes = array()) {
        ...
        echo '<a href="#" id="'. $this->_trigger_id($id) . '">' .
            '<img align="middle" border="0" src="' . $this->calendar_lib_path . 'themes/img.gif" alt="" /></a>';
