...
var calDateYearField;
var calDateMonthField;
var calDateDayField;
...
// FUNCTION:    setDateFieldYMD
// PURPOSE:     set up the initial date to the year,month and day field
//              passed in and disable selection of any date prior to the
//              date specified by the (hidden) startDateField
// PARAMETERS:  yearDateField,
//              monthDateField,
//              dayDateField   - the year,month and day fields to use for the calendar's
//                               initial date (datatype: text field)
//              startDateField - the first valid date that may be selected
//                               (datatype: hidden field, always US format mm/dd/yyyy)
function setDateFieldYMD(yearDateField,monthDateField,dayDateField,startDateField) {

    if ( startDateField )
        {
	    startDate.setTime(Date.parse(startDateField.value));
        }

    // ASSIGN THE INCOMING FIELD OBJECT TO A GLOBAL VARIABLE
    // calDateField = dateField;
    calDateYearField = yearDateField;
    calDateMonthField = monthDateField;
    calDateDayField = dayDateField;

    // GET THE VALUE OF THE INCOMING FIELD
    // inDate = dateField.value;
    inDate = calDateMonthField.value + "/" + calDateDayField.value + "/" + calDateYearField.value;
    ...
    // If the format is mm/dd/yyyy, then make sure user entered a four digit year
    if ( calDateFormat == "mm/dd/yyyy" )
        {
        DateArray = inDate.split('/');
        if ( DateArray.length == 3 )
            {
            tmpInt = parseInt(DateArray[2],10);
            if ( isNaN(tmpInt) == false )
                ...
                if ( tmpInt < 100 )
                    ...
                    if ( tmpInt < 30 )
                        ...
                        tmpInt = tmpInt + 1900;
                    inDate = DateArray[0] + "/" + DateArray[1] + "/" + tmpInt;
                    }
                }
            }
        }
...
function showCalendar(dateField,startDateField) {

    // SET INITIAL VALUE OF THE DATE FIELD AND CREATE TOP AND BOTTOM FRAMES
    setDateField(dateField,startDateField);

    // USE THE JAVASCRIPT-GENERATED DOCUMENTS (calDocTop, calDocBottom) IN THE FRAMESET
    calDocFrameset =
        "<HTML><HEAD><TITLE>AdBase RunDates</TITLE></HEAD>\n" +
...
function returnDate(inDay)
{
    ...
    // SET THE VALUE OF THE FIELDS THAT WERE PASSED TO THE CALENDAR
    //calDateField.value = outDate;
    calDateYearField.value = "" + year;
    calDateMonthField.value = "" + month;
    calDateDayField.value = "" + day;

    // GIVE FOCUS BACK TO THE DATE FIELD
    //  Use the month field of our year, month, day trio
    calDateMonthField.focus();
    //calDateField.focus();
