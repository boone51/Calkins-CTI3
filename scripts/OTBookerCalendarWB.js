...
//		invalidDates	- invalid run dates user can select (datatype: string)
...
function setDateFields(dateField,insertsField,startDateField,validDates,
			invalidDates,DowMask,deadlineDates,selectedDates,mandatoryDates) {
...
// vlink, alink, link, text
	...
	if ( (startDate.getFullYear() > year) ||
	    ...
	    if ( isSelectedDate(month,currentDay,year) )
	       {
...
function applyButtonClick() {
	...
	if ( theSelectedDateStr.length != 0 )
	    ...
	    theSelectedDateStr += "|";
            ...
            // Track 23427
            // Using the long format will be too long for the URL (2083 max for IE) so we are switching to the "short format"

            // theSelectedDateStr += selectedDates_a[i].toLocaleString();
            theSelectedDateStr += (selectedDates_a[i].getMonth()+1)+"/"+selectedDates_a[i].getDate()+"/"+selectedDates_a[i].getFullYear();
    	}
	...
	outDate += (selectedDates_a[firstRunDateIndex].getMonth()+1)+"/"+selectedDates_a[firstRunDateIndex].getDate()+"/"+selectedDates_a[firstRunDateIndex].getFullYear();
	}
    ...
    parent.parent.SendParameters('rsconstraints.ot', 'run_mode=RunScheduleWB', 'command=ApplyRSDates',
	'RSSelectedDates='+theSelectedDateStr, 'RSNumberOfInserts=' + selectedDates_a.length,
	'RSStartDate='+outDate);
}
