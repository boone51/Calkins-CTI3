...
function setDateField(dateField,startDateField) {

    if ( startDateField )
        {
        startDateStr = startDateField.value;
        DateArray = startDateStr.split('/');
        if ( DateArray.length == 3 )
            {
            startDate.setFullYear(parseInt(DateArray[2],10));
            startDate.setMonth(parseInt(DateArray[0],10)-1);
            startDate.setDate(parseInt(DateArray[1],10));
            }
        }
...
function writeCalendar() {
    ...
    // NOTE: Uncommenting this line would make IE/MacOSX stop working!
    //       Basically 'calDocBottom' can't be used any more
    // calDocBottom = buildBottomCalFrame();
